  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className={`truncate text-base ${isArc ? 'text-primary' : 'text-fg'}`}>
          {chain.fullName}
        </div>
      ...
      {isSoon ? (
        <span className="rounded bg-warn/12 px-2 py-0.5 text-[10px] uppercase tracking-wide text-warn">
          Soon
        </span>
      ) : (
        <div className={`text-right font-mono text-base ${has ? 'text-fg' : 'text-muted'}`}>
          {formatted} <span className="text-[11px] text-muted">USDC</span>
        </div>
      )}
    </li>
  );
